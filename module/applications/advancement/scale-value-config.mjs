...
export default class ScaleValueConfig extends AdvancementConfig {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["mc3e", "advancement", "scale-value", "two-column"],
      template: "systems/mc3e/templates/advancement/scale-value-config.hbs",
      width: 540
    });
  }
  ...
  getData() {
    ...
    return foundry.utils.mergeObject(super.getData(), {
      ...
      movementUnits: CONFIG.mc3e.movementUnits
    });
  }
  ...
  _prepareLevelData() {
    let lastValue = null;
    return Array.fromRange(CONFIG.mc3e.maxLevel + 1).slice(1).reduce((obj, level) => {
  ...
  _onIdentifierHintCopy(event) {
    ...
    game.tooltip.activate(event.target, {text: game.i18n.localize("mc3e.IdentifierCopied"), direction: "UP"});
  }
  ...
  async _updateObject(event, formData) {
    ...
    if ( typeChange && (updates.configuration.type !== this.advancement.configuration.type) ) {
      ...
      await this.advancement.update(Array.fromRange(CONFIG.mc3e.maxLevel, 1).reduce((obj, lvl) => {
