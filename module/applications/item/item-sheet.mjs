...
export default class ItemSheet5e extends ItemSheet {
  ...
  /** @inheritdoc */
  get template() {
    return `systems/mc3e/templates/items/${this.item.type}.hbs`;
  }
  ...
  async getData(options) {
    ...
    switch ( item.type ) {
      case "feat":
        const featureType = CONFIG.mc3e.featureTypes[item.system.type?.value];
        if ( featureType ) {
        ...
        break;
      case "spell":
        context.spellComponents = {...CONFIG.mc3e.spellComponents, ...CONFIG.mc3e.spellTags};
        break;
    }
  ...
  async _getItemBaseTypes() {
    ...
    const baseIds = CONFIG.mc3e[`${type}Ids`];
  ...
  _getItemConsumptionTargets() {
    ...
    // Hit Dice
      ...
      return {
        smallest: game.i18n.localize("mc3e.ConsumeHitDiceSmallest"),
        ...CONFIG.mc3e.hitDieTypes.reduce((obj, hd) => { obj[hd] = hd; return obj; }, {}),
        largest: game.i18n.localize("mc3e.ConsumeHitDiceLargest")
      };
    }
      ...
      return actor.items.reduce((obj, i) => {
        ...
        if ( uses.per && uses.max ) {
          const label = uses.per === "charges"
            ? ` (${game.i18n.format("mc3e.AbilityUseChargesLabel", {value: uses.value})})`
            : ` (${game.i18n.format("mc3e.AbilityUseConsumableLabel", {max: uses.max, per: uses.per})})`;
          obj[i.id] = i.name + label;
        }

        // Recharging items
        const recharge = i.system.recharge || {};
        if ( recharge.value ) obj[i.id] = `${i.name} (${game.i18n.format("mc3e.Recharge")})`;
        return obj;
      }, {});
    }
    else return {};
  ...
  _getItemStatus() {
    switch ( this.item.type ) {
      case "class":
        return game.i18n.format("mc3e.LevelCount", {ordinal: this.item.system.levels.ordinalString()});
      case "equipment":
      case "weapon":
        return game.i18n.localize(this.item.system.equipped ? "mc3e.Equipped" : "mc3e.Unequipped");
      case "feat":
        const typeConfig = CONFIG.mc3e.featureTypes[this.item.system.type.value];
        if ( typeConfig?.subtypes ) return typeConfig.subtypes[this.item.system.type.subtype] ?? null;
        break;
      case "spell":
        return CONFIG.mc3e.spellPreparationModes[this.item.system.preparation];
      case "tool":
        return game.i18n.localize(this.item.system.proficient ? "mc3e.Proficient" : "mc3e.NotProficient");
    }
    return null;
  }
  ...
  _getItemProperties() {
    ...
    switch ( this.item.type ) {
      case "equipment":
        props.push(CONFIG.mc3e.equipmentTypes[this.item.system.armor.type]);
      ...
      case "weapon":
        for ( const [k, v] of Object.entries(this.item.system.properties) ) {
          if ( v === true ) props.push(CONFIG.mc3e.weaponProperties[k]);
        }
        break;
    }
    ...
    if ( this.item.system.actionType ) {
      props.push(CONFIG.mc3e.itemActionTypes[this.item.system.actionType]);
    }
  ...
  _getSubmitData(updateData={}) {
    ...
    if ( uses?.max ) {
      ...
      if ( !maxRoll.isDeterministic ) {
        ...
        return ui.notifications.error(game.i18n.format("mc3e.FormulaCannotContainDiceError", {
          name: game.i18n.localize("mc3e.LimitedUses")
        }));
      }
    }
    ...
    if ( duration?.value ) {
      ...
      if ( !durationRoll.isDeterministic ) {
        ...
        return ui.notifications.error(game.i18n.format("mc3e.FormulaCannotContainDiceError", {
          name: game.i18n.localize("mc3e.Duration")
        }));
      }
    }

    // Check class identifier
    if ( formData.system?.identifier && !mc3e.utils.validators.isValidIdentifier(formData.system.identifier) ) {
      ...
      return ui.notifications.error(game.i18n.localize("mc3e.IdentifierError"));
    }
  ...
  activateListeners(html) {
     ...
     * @function mc3e.getItemAdvancementContext
     * @memberof hookEvents
     * @param {jQuery} html                      The HTML element to which the context options are attached.
     * @param {ContextMenuEntry[]} entryOptions  The context menu entries.
     */
    Hooks.call("mc3e.getItemAdvancementContext", html, contextOptions);
  ...
  _getAdvancementContextMenuOptions() {
    ...
    return [
      {
        name: "mc3e.AdvancementControlEdit",
        icon: "<i class='fas fa-edit fa-fw'></i>",
        condition,
        callback: li => this._onAdvancementAction(li[0], "edit")
      },
      {
        name: "mc3e.AdvancementControlDuplicate",
      ...
      {
        name: "mc3e.AdvancementControlDelete",
  ...
  _onDrop(event) {
     ...
     * @function mc3e.dropItemSheetData
     * @memberof hookEvents
     ...
     */
    const allowed = Hooks.call("mc3e.dropItemSheetData", item, this, data);
  ...
  async _onDropAdvancement(event, data) {
    ...
    if ( this.item.isEmbedded && !game.settings.get("mc3e", "disableAdvancements") ) {
  ...
  _onConfigureTraits(event) {
    ...
    switch (a.dataset.options) {
      case "saves":
        options.choices = CONFIG.mc3e.abilities;
        options.valueKey = null;
        break;
      case "skills.choices":
        options.choices = CONFIG.mc3e.skills;
        options.valueKey = null;
        options.labelKey = "label";
        break;
      case "skills":
        const skills = this.item.system.skills;
        const choices = skills.choices?.length ? skills.choices : Object.keys(CONFIG.mc3e.skills);
        options.choices = Object.fromEntries(Object.entries(CONFIG.mc3e.skills).filter(([s]) => choices.includes(s)));
  ...
  _onAdvancementAction(target, action) {
    ...
    switch (action) {
      case "add": return game.mc3e.applications.advancement.AdvancementSelection.createDialog(this.item);
