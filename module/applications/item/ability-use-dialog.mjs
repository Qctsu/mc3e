...
export default class AbilityUseDialog extends Dialog {
  constructor(item, dialogData={}, options={}) {
    ...
    this.options.classes = ["mc3e", "dialog"];
  ...
  static async create(item) {
    ...
    const data = {
      item: item,
      title: game.i18n.format("mc3e.AbilityUseHint", {type: game.i18n.localize(`ITEM.Type${item.type.capitalize()}`), name: item.name}),
    ...
    const html = await renderTemplate("systems/mc3e/templates/apps/ability-use.hbs", data);

    // Create the Dialog and return data as a Promise
    const icon = data.isSpell ? "fa-magic" : "fa-fist-raised";
    const label = game.i18n.localize(`mc3e.AbilityUse${data.isSpell ? "Cast" : "Use"}`);
    return new Promise(resolve => {
      const dlg = new this(item, {
        title: `${item.name}: ${game.i18n.localize("mc3e.AbilityUseConfig")}`,
        content: html,
        buttons: {
          use: {
  ...
  static _getSpellData(actorData, itemData, data) {
    ...
    const consumeSpellSlot = (lvl > 0) && CONFIG.mc3e.spellUpcastModes.includes(itemData.preparation.mode);
    ...
    const spellLevels = Array.fromRange(10).reduce((arr, i) => {
      if ( i < lvl ) return arr;
      const label = CONFIG.mc3e.spellLevels[i];
      ...
      arr.push({
        level: i,
        label: i > 0 ? game.i18n.format("mc3e.SpellLevelSlot", {level: label, n: slots}) : label,
        canCast: max > 0,
        hasSlots: slots > 0
      });
      return arr;
    ...
    if (pact.level >= lvl) {
      spellLevels.push({
        level: "pact",
        label: `${game.i18n.format("mc3e.SpellLevelPact", {level: pact.level, n: pact.value})}`,
        canCast: true,
        hasSlots: pact.value > 0
      });
    }
    const canCast = spellLevels.some(l => l.hasSlots);
    if ( !canCast ) data.errors.push(game.i18n.format("mc3e.SpellCastNoSlots", {
      level: CONFIG.mc3e.spellLevels[lvl],
      name: data.item.name
    }));
  ...
  static _getAbilityUseNote(item, uses, recharge) {
    ...
    if ( quantity <= 0 ) return game.i18n.localize("mc3e.AbilityUseUnavailableHint");

    // Abilities which use Recharge
    if ( recharge.value ) {
      return game.i18n.format(recharge.charged ? "mc3e.AbilityUseChargedHint" : "mc3e.AbilityUseRechargeHint", {
    ...
    if ( item.type === "consumable" ) {
      let str = "mc3e.AbilityUseNormalHint";
      if ( uses.value > 1 ) str = "mc3e.AbilityUseConsumableChargeHint";
      else if ( item.system.quantity === 1 && uses.autoDestroy ) str = "mc3e.AbilityUseConsumableDestroyHint";
      else if ( item.system.quantity > 1 ) str = "mc3e.AbilityUseConsumableQuantityHint";
      return game.i18n.format(str, {
        type: game.i18n.localize(`mc3e.Consumable${item.system.consumableType.capitalize()}`),
        value: uses.value,
        quantity: item.system.quantity,
        max: uses.max,
        per: CONFIG.mc3e.limitedUsePeriods[uses.per]
      });
    }

    // Other Items
    else {
      return game.i18n.format("mc3e.AbilityUseNormalHint", {
        type: game.i18n.localize(`ITEM.Type${item.type.capitalize()}`),
        value: uses.value,
        max: uses.max,
        per: CONFIG.mc3e.limitedUsePeriods[uses.per]
