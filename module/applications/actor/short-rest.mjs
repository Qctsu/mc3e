...
export default class ShortRestDialog extends Dialog {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      template: "systems/mc3e/templates/apps/short-rest.hbs",
      classes: ["mc3e", "dialog"]
    });
  }
  ...
  getData() {
    ...
    // Determine rest type
    const variant = game.settings.get("mc3e", "restVariant");
  ...
  static async shortRestDialog({ actor }={}) {
    return new Promise((resolve, reject) => {
      const dlg = new this(actor, {
        title: `${game.i18n.localize("mc3e.ShortRest")}: ${actor.name}`,
        buttons: {
          rest: {
            icon: '<i class="fas fa-bed"></i>',
            label: game.i18n.localize("mc3e.Rest"),
            callback: html => {
              let newDay = false;
              if ( game.settings.get("mc3e", "restVariant") !== "epic" ) {
