...
export default class ActorSheet5eCharacter extends ActorSheet5e {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["mc3e", "sheet", "actor", "character"]
    });
  }
  ...
  async getData(options={}) {
    ...
    context.resources = ["primary", "secondary", "tertiary"].reduce((arr, r) => {
      ...
      res.name = r;
      res.placeholder = game.i18n.localize(`mc3e.Resource${r.titleCase()}`);
    ...
    return foundry.utils.mergeObject(context, {
      disableExperience: game.settings.get("mc3e", "disableExperienceTracking"),
      ...
      weightUnit: game.i18n.localize(`mc3e.Abbreviation${
        game.settings.get("mc3e", "metricWeightUnits") ? "Kgs" : "Lbs"}`),
  ...
  _prepareItems(context) {
    ...
    let {items, spells, feats, backgrounds, classes, subclasses} = context.items.reduce((obj, item) => {
      ...
      ctx.attunement = {
        [CONFIG.mc3e.attunementTypes.REQUIRED]: {
          icon: "fa-sun",
          cls: "not-attuned",
          title: "mc3e.AttunementRequired"
        },
        [CONFIG.mc3e.attunementTypes.ATTUNED]: {
          icon: "fa-sun",
          cls: "attuned",
          title: "mc3e.AttunementAttuned"
        }
    ...
    const maxLevelDelta = CONFIG.mc3e.maxLevel - this.actor.system.details.level;
    classes = classes.reduce((arr, cls) => {
      const ctx = context.itemContext[cls.id] ??= {};
      ctx.availableLevels = Array.fromRange(CONFIG.mc3e.maxLevel + 1).slice(1).map(level => {
    ...
    for ( const subclass of subclasses ) {
      feats.push(subclass);
      const message = game.i18n.format("mc3e.SubclassMismatchWarn", {
    ...
    const features = {
      ...
      active: {
        label: "mc3e.FeatureActive", items: [],
        hasActions: true, dataset: {type: "feat", "activation.type": "action"} },
      passive: {
        label: "mc3e.FeaturePassive", items: [],
  ...
  _prepareItemToggleState(item, context) {
    if ( item.type === "spell" ) {
      ...
      if ( isAlways ) context.toggleTitle = CONFIG.mc3e.spellPreparationModes.always;
      else if ( isPrepared ) context.toggleTitle = CONFIG.mc3e.spellPreparationModes.prepared;
      else context.toggleTitle = game.i18n.localize("mc3e.SpellUnprepared");
    }
    else {
      const isActive = !!item.system.equipped;
      context.toggleClass = isActive ? "active" : "";
      context.toggleTitle = game.i18n.localize(isActive ? "mc3e.Equipped" : "mc3e.Unequipped");
  ...
  async _onDropSingleItem(itemData) {
    ...
    if ( itemData.type === "class" ) {
      ...
      itemData.system.levels = Math.min(itemData.system.levels, CONFIG.mc3e.maxLevel - charLevel);
    ...
    // If a subclass is dropped, ensure it doesn't match another subclass with the same identifier
      ...
      if ( other ) {
        const err = game.i18n.format("mc3e.SubclassDuplicateError", {identifier: other.identifier});
        ui.notifications.error(err);
        return false;
      }
      const cls = this.actor.itemTypes.class.find(i => i.identifier === itemData.system.classIdentifier);
      if ( cls && cls.subclass ) {
        const err = game.i18n.format("mc3e.SubclassAssignmentError", {class: cls.name, subclass: cls.subclass.name});
        ui.notifications.error(err);
        return false;
      }
    }
