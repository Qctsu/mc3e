...
export default class ActorSheet5eVehicle extends ActorSheet5e {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["mc3e", "sheet", "actor", "vehicle"]
    });
  }
  ...
  _computeEncumbrance(totalWeight, actorData) {
    ...
    totalWeight /= game.settings.get("mc3e", "metricWeightUnits")
      ? CONFIG.mc3e.encumbrance.vehicleWeightMultiplier.metric
      : CONFIG.mc3e.encumbrance.vehicleWeightMultiplier.imperial;
  ...
  _prepareCrewedItem(item, context) {
    ...
    context.toggleTitle = game.i18n.localize(`mc3e.${isCrewed ? "Crewed" : "Uncrewed"}`);

    // Handle crew actions
    if ( item.type === "feat" && item.system.activation.type === "crew" ) {
      context.cover = game.i18n.localize(`mc3e.${item.system.cover ? "CoverTotal" : "None"}`);
  ...
  _prepareItems(context) {
    const cargoColumns = [{
      label: game.i18n.localize("mc3e.Quantity"),
      css: "item-qty",
      property: "quantity",
      editable: "Number"
    }];
    ...
    const equipmentColumns = [{
      ...
      label: game.i18n.localize("mc3e.AC"),
      css: "item-ac",
      property: "system.armor.value"
    }, {
      label: game.i18n.localize("mc3e.HP"),
      css: "item-hp",
      property: "system.hp.value",
      editable: "Number"
    }, {
      label: game.i18n.localize("mc3e.Threshold"),
      css: "item-threshold",
      property: "threshold"
    }];
    ...
    const features = {
      actions: {
        label: game.i18n.localize("mc3e.ActionPl"),
        items: [],
        hasActions: true,
        crewable: true,
      ...
      passive: {
        label: game.i18n.localize("mc3e.Features"),
        items: [],
        dataset: {type: "feat"}
      },
      reactions: {
        label: game.i18n.localize("mc3e.ReactionPl"),
        items: [],
    ...
    const cargo = {
      crew: {
        label: game.i18n.localize("mc3e.VehicleCrew"),
      ...
      passengers: {
        label: game.i18n.localize("mc3e.VehiclePassengers"),
      ...
      cargo: {
        label: game.i18n.localize("mc3e.VehicleCargo"),
        items: [],
        dataset: {type: "loot"},
        columns: [{
          label: game.i18n.localize("mc3e.Quantity"),
          ...
          label: game.i18n.localize("mc3e.Price"),
          css: "item-price",
          property: "system.price.value",
          editable: "Number"
        }, {
          label: game.i18n.localize("mc3e.Weight"),
          css: "item-weight",
    ...
    for ( const item of context.items ) {
      ...
      const isCargo = item.flags.mc3e?.vehicleCargo === true;
      if ( isCargo ) {
  ...
  async _onDropSingleItem(itemData) {
    ...
    foundry.utils.setProperty(itemData, "flags.mc3e.vehicleCargo", isCargo);
