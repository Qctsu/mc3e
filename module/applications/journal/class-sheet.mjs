...
export default class JournalClassPageSheet extends JournalPageSheet {
  ...
  /** @inheritdoc */
  get template() {
    return `systems/mc3e/templates/journal/page-class-${this.isEditable ? "edit" : "view"}.hbs`;
  }
  ...
  async _getTable(item, initialLevel=1) {
    ...
    const headers = [[{content: game.i18n.localize("mc3e.Level")}]];
    if ( item.type === "class" ) headers[0].push({content: game.i18n.localize("mc3e.ProficiencyBonus")});
    if ( hasFeatures ) headers[0].push({content: game.i18n.localize("mc3e.Features")});
    ...
    const rows = [];
    for ( const level of Array.fromRange((CONFIG.mc3e.maxLevel - (initialLevel - 1)), initialLevel) ) {
      const features = [];
  ...
  async _getSpellProgression(item) {
    ...
    if ( spellcasting.type === "leveled" ) {
      const spells = {};
      const maxSpellLevel = CONFIG.mc3e.SPELL_SLOT_TABLE[CONFIG.mc3e.SPELL_SLOT_TABLE.length - 1].length;
      Array.fromRange(maxSpellLevel, 1).forEach(l => spells[`spell${l}`] = {});

      let largestSlot;
      for ( const level of Array.fromRange(CONFIG.mc3e.maxLevel, 1).reverse() ) {
      ...
      table.headers = [
        [{content: game.i18n.localize("JOURNALENTRYPAGE.mc3e.Class.SpellSlotsPerSpellLevel"), colSpan: largestSlot}],
      ...
      table.headers = [[
        { content: game.i18n.localize("JOURNALENTRYPAGE.mc3e.Class.SpellSlots") },
        { content: game.i18n.localize("JOURNALENTRYPAGE.mc3e.Class.SpellSlotLevel") }
      ]];
      table.cols = [{class: "spellcasting", span: 2}];

      // Loop through each level, gathering "Spell Slots" & "Slot Level" for each one
      for ( const level of Array.fromRange(CONFIG.mc3e.maxLevel, 1) ) {
       ...
       * The actual hook names include the spellcasting type (e.g. `mc3e.buildPsionicSpellcastingTable`).
       ...
       * @function mc3e.buildSpellcastingTable
       * @memberof hookEvents
       */
      Hooks.callAll(
        `mc3e.build${spellcasting.type.capitalize()}SpellcastingTable`, table, item, spellcasting
      );
    }
  ...
  async _getOptionalTable(item) {
    const headers = [[
      { content: game.i18n.localize("mc3e.Level") },
      { content: game.i18n.localize("mc3e.Features") }
    ]];
