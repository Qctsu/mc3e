...
export default class Item5e extends Item {
  ...
  get hasAreaTarget() {
    const target = this.system.target;
    return target && (target.type in CONFIG.mc3e.areaTargetTypes);
  }
   ...
   */
  get isArmor() {
    return this.system.armor?.type in CONFIG.mc3e.armorTypes;
  }
   ...
   * @property {string} type              Spellcasting type as defined in ``CONFIG.mc3e.spellcastingTypes`.
  ...
  get spellcasting() {
    ...
    if ( CONFIG.mc3e.spellcastingTypes[finalSC.progression] ) finalSC.type = finalSC.progression;
    else finalSC.type = Object.entries(CONFIG.mc3e.spellcastingTypes).find(([type, data]) => {
  ...
  get areEffectsSuppressed() {
    ...
    return this.system.attunement === CONFIG.mc3e.attunementTypes.REQUIRED;
  }
   ...
   */
  _prepareEquipment() {
    this.labels.armor = this.system.armor.value ? `${this.system.armor.value} ${game.i18n.localize("mc3e.AC")}` : "";
  }
  ...
  _prepareFeat() {
    ...
    const types = CONFIG.mc3e.abilityActivationTypes;
    if ( act?.type === types.legendary ) this.labels.featType = game.i18n.localize("mc3e.LegendaryActionLabel");
    else if ( act?.type === types.lair ) this.labels.featType = game.i18n.localize("mc3e.LairActionLabel");
    else if ( act?.type ) {
      this.labels.featType = game.i18n.localize(this.system.damage.length ? "mc3e.Attack" : "mc3e.Action");
    }
    else this.labels.featType = game.i18n.localize("mc3e.Passive");
  }
   ...
   */
  _prepareSpell() {
    const tags = Object.fromEntries(Object.entries(CONFIG.mc3e.spellTags).map(([k, v]) => {
      v.tag = true;
      return [k, v];
    }));
    const attributes = {...CONFIG.mc3e.spellComponents, ...tags};
    this.system.preparation.mode ||= "prepared";
    this.labels.level = CONFIG.mc3e.spellLevels[this.system.level];
    this.labels.school = CONFIG.mc3e.spellSchools[this.system.school];
  ...
  _prepareActivation() {
    ...
    this.labels.recharge = `${game.i18n.localize("mc3e.Recharge")} [${chgSuffix}]`;
  }
  ...
  _prepareAction() {
    ...
    if ( dmg.parts ) {
      const types = CONFIG.mc3e.damageTypes;
  ...
  _prepareAdvancement() {
    ...
    this.advancement = {
      ...
      byLevel: Object.fromEntries(
        Array.fromRange(CONFIG.mc3e.maxLevel + 1).slice(minAdvancementLevel).map(l => [l, []])
      ),
      byType: {},
  ...
  prepareFinalAttributes() {
    ...
    if ( "actionType" in this.system ) {
      this.labels.abilityCheck = game.i18n.format("mc3e.AbilityPromptTitle", {
        ability: CONFIG.mc3e.abilities[this.system.ability]
      });
  ...
  getDerivedDamageLabel() {
    ...
    const damageLabels = { ...CONFIG.mc3e.damageTypes, ...CONFIG.mc3e.healingTypes };
  ...
  getSaveDC() {
    ...
    // Update labels
    const abl = CONFIG.mc3e.abilities[save.ability] ?? "";
    this.labels.save = game.i18n.format("mc3e.SaveDC", {dc: save.dc || "", ability: abl});
    return save.dc;
  }
  ...
  getCriticalThreshold() {
    const actorFlags = this.actor.flags.mc3e || {};
    if ( !this.hasAttack ) return null;
  ...
  prepareMaxUses() {
    ...
    if ( this.isOwned && !Number.isNumeric(max) ) {
      const property = game.i18n.localize("mc3e.UsesMax");
      try {
        const rollData = this.getRollData({ deterministic: true });
        max = Roll.safeEval(this.replaceFormulaData(max, rollData, { property }));
      } catch(e) {
        const message = game.i18n.format("mc3e.FormulaMalformedError", { property, name: this.name });
  ...
  prepareDurationValue() {
    ...
    if ( this.isOwned && !Number.isNumeric(value) ) {
      const property = game.i18n.localize("mc3e.Duration");
      try {
        const rollData = this.getRollData({ deterministic: true });
    ...
    this.labels.duration = [duration.value, CONFIG.mc3e.timePeriods[duration.units]].filterJoin(" ");
  }
  ...
  replaceFormulaData(formula, data, { property }) {
    ...
    if ( (missingReferences.size > 0) && this.actor ) {
      ...
      const message = game.i18n.format("mc3e.FormulaMissingReferenceWarn", {
  ...
  async use(config={}, options={}) {
    ...
    options = foundry.utils.mergeObject({
      ...
      createMessage: true,
      "flags.mc3e.use": {type: this.type, itemId: this.id, itemUuid: this.uuid}
    }, options);

    // Reference aspects of the item data necessary for usage
    const resource = is.consume || {};        // Resource consumption
    const isSpell = item.type === "spell";    // Does the item require a spell slot?
    const requireSpellSlot = isSpell && (is.level > 0) && CONFIG.mc3e.spellUpcastModes.includes(is.preparation.mode);
     ...
     */
    if ( Hooks.call("mc3e.preUseItem", item, config, options) === false ) return;
    ...
    if ( isSpell ) foundry.utils.mergeObject(options.flags, {"mc3e.use.spellLevel": item.system.level});

    /**
     * A hook event that fires before an item's resource consumption has been calculated.
     * @function mc3e.preItemUsageConsumption
     * @memberof hookEvents
     * @param {Item5e} item                  Item being used.
     * @param {ItemUseConfiguration} config  Configuration data for the item usage being prepared.
     * @param {ItemUseOptions} options       Additional options used for configuring item usage.
     * @returns {boolean}                    Explicitly return `false` to prevent item from being used.
     */
    if ( Hooks.call("mc3e.preItemUsageConsumption", item, config, options) === false ) return;
     ...
     * @function mc3e.itemUsageConsumption
     * @memberof hookEvents
     * @param {Item5e} item                     Item being used.
     * @param {ItemUseConfiguration} config     Configuration data for the item usage being prepared.
     * @param {ItemUseOptions} options          Additional options used for configuring item usage.
     ...
     * @returns {boolean}                       Explicitly return `false` to prevent item from being used.
     */
    if ( Hooks.call("mc3e.itemUsageConsumption", item, config, options, usage) === false ) return;
     ...
     */
    Hooks.callAll("mc3e.useItem", item, config, options, templates ?? null);
  ...
  _getUsageUpdates({
    ...
    if ( consumeRecharge ) {
      ...
      if ( recharge.charged === false ) {
        ui.notifications.warn(game.i18n.format("mc3e.ItemNoUses", {name: this.name}));
        return false;
      }
    ...
    if ( consumeSpellSlot && consumeSpellLevel ) {
      ...
      if ( spells === 0 ) {
        const labelKey = consumeSpellLevel === "pact" ? "mc3e.SpellProgPact" : `mc3e.SpellLevel${this.system.level}`;
        const label = game.i18n.localize(labelKey);
        ui.notifications.warn(game.i18n.format("mc3e.SpellCastNoSlots", {name: this.name, level: label}));
        return false;
      }
  ...
  _handleConsumeResource(itemUpdates, actorUpdates, resourceUpdates) {
    ...
    // No consumed target
    const typeLabel = CONFIG.mc3e.abilityConsumptionTypes[consume.type];
    if ( !consume.target ) {
      ui.notifications.warn(game.i18n.format("mc3e.ConsumeWarningNoResource", {name: this.name, type: typeLabel}));
      return false;
    }
    ...
    if ( resource === undefined ) {
      ui.notifications.warn(game.i18n.format("mc3e.ConsumeWarningNoSource", {name: this.name, type: typeLabel}));
      return false;
    }
    ...
    if ( remaining < 0 ) {
      ui.notifications.warn(game.i18n.format("mc3e.ConsumeWarningNoQuantity", {name: this.name, type: typeLabel}));
      return false;
    }
  ...
  async displayCard(options={}) {
    ...
    const html = await renderTemplate("systems/mc3e/templates/chat/item-card.hbs", templateData);
    ...
    if ( (this.type === "consumable") && !this.actor.items.has(this.id) ) {
      chatData.flags["mc3e.itemData"] = templateData.item.toObject();
    }
     ...
     * @param {ItemUseOptions} options  Options which configure the display of the item chat card.
     */
    Hooks.callAll("mc3e.preDisplayCard", this, chatData, options);
     ...
     */
    Hooks.callAll("mc3e.displayCard", this, card);
  ...
  async getChatData(htmlOptions={}) {
    ...
    if ( data.hasOwnProperty("equipped") && !["loot", "tool"].includes(this.type) ) {
      if ( data.attunement === CONFIG.mc3e.attunementTypes.REQUIRED ) {
        props.push(CONFIG.mc3e.attunements[CONFIG.mc3e.attunementTypes.REQUIRED]);
      }
      props.push(
        game.i18n.localize(data.equipped ? "mc3e.Equipped" : "mc3e.Unequipped"),
        game.i18n.localize(data.proficient ? "mc3e.Proficient" : "mc3e.NotProficient")
      );
    }
  ...
  _consumableChatData(data, labels, props) {
    props.push(
      CONFIG.mc3e.consumableTypes[data.consumableType],
      `${data.uses.value}/${data.uses.max} ${game.i18n.localize("mc3e.Charges")}`
    );
  ...
  _equipmentChatData(data, labels, props) {
    props.push(
      CONFIG.mc3e.equipmentTypes[data.armor.type],
      labels.armor || null,
      data.stealth ? game.i18n.localize("mc3e.StealthDisadvantage") : null
    );
  }
  ...
  _lootChatData(data, labels, props) {
    props.push(
      ...
      data.weight ? `${data.weight} ${game.i18n.localize("mc3e.AbbreviationLbs")}` : null
    );
  }
  ...
  _toolChatData(data, labels, props) {
    props.push(
      CONFIG.mc3e.abilities[data.ability] || null,
      CONFIG.mc3e.proficiencyLevels[data.proficient || 0]
    );
  }
  ...
  _weaponChatData(data, labels, props) {
    props.push(
      CONFIG.mc3e.weaponTypes[data.weaponType]
    );
  }
  ...
  async rollAttack(options={}) {
    const flags = this.actor.flags.mc3e ?? {};
    if ( !this.hasAttack ) throw new Error("You may not place an Attack Roll with this Item.");
    let title = `${this.name} - ${game.i18n.localize("mc3e.AttackRoll")}`;
    ...
    const elvenAccuracy = (flags.elvenAccuracy
      && CONFIG.mc3e.characterFlags.elvenAccuracy.abilities.includes(this.abilityMod)) || undefined;
    ...
    const rollConfig = foundry.utils.mergeObject({
      ...
      messageData: {
        "flags.mc3e.roll": {type: "attack", itemId: this.id, itemUuid: this.uuid},
  ...
  async rollDamage({critical, event=null, spellLevel=null, versatile=false, options={}}={}) {
    ...
    const messageData = {
      "flags.mc3e.roll": {type: "damage", itemId: this.id, itemUuid: this.uuid},
      speaker: ChatMessage.getSpeaker({actor: this.actor})
    };
    ...
    const actionFlavor = game.i18n.localize(this.system.actionType === "heal" ? "mc3e.Healing" : "mc3e.DamageRoll");
    ...
    if ( versatile && dmg.versatile ) {
      ...
      messageData["flags.mc3e.roll"].versatile = true;
    }
    ...
    if ( this.system.actionType === "mwak" ) {
      rollConfig.criticalBonusDice = this.actor.getFlag("mc3e", "meleeCriticalDamageDice") ?? 0;
    }
  ...
  async rollFormula({spellLevel}={}) {
     ...
     * @returns {boolean}                   Explicitly return false to prevent the roll from being performed.
     */
    if ( Hooks.call("mc3e.preRollFormula", this, rollConfig) === false ) return;
    ...
    if ( rollConfig.chatMessage ) {
      roll.toMessage({
        speaker: ChatMessage.getSpeaker({actor: this.actor}),
        flavor: `${this.name} - ${game.i18n.localize("mc3e.OtherFormula")}`,
        rollMode: game.settings.get("core", "rollMode"),
        messageData: {"flags.mc3e.roll": {type: "other", itemId: this.id, itemUuid: this.uuid}}
      });
    }

    /**
     * A hook event that fires after a formula has been rolled for an Item.
     * @function mc3e.rollFormula
     * @memberof hookEvents
     * @param {Item5e} item  Item for which the roll was performed.
     * @param {Roll} roll    The resulting roll.
     */
    Hooks.callAll("mc3e.rollFormula", this, roll);
  ...
  async rollRecharge() {
     ...
     * @returns {boolean}                   Explicitly return false to prevent the roll from being performed.
     */
    if ( Hooks.call("mc3e.preRollRecharge", this, rollConfig) === false ) return;

    const roll = await new Roll(rollConfig.formula, rollConfig.data).roll({async: true});
    const success = roll.total >= rollConfig.target;

    if ( rollConfig.chatMessage ) {
      const resultMessage = game.i18n.localize(`mc3e.ItemRecharge${success ? "Success" : "Failure"}`);
      roll.toMessage({
        flavor: `${game.i18n.format("mc3e.ItemRechargeCheck", {name: this.name})} - ${resultMessage}`,
     ...
     * @function mc3e.rollRecharge
     * @memberof hookEvents
     * @param {Item5e} item  Item for which the roll was performed.
     * @param {Roll} roll    The resulting roll.
     * @returns {boolean}    Explicitly return false to prevent the item from being recharged.
     */
    if ( Hooks.call("mc3e.rollRecharge", this, roll) === false ) return roll;
  ...
  async rollToolCheck(options={}) {
    ...
    const title = `${this.name} - ${game.i18n.localize("mc3e.ToolCheck")}`;
    ...
    const rollConfig = foundry.utils.mergeObject({
      ...
      chooseModifier: true,
      halflingLucky: this.actor.getFlag("mc3e", "halflingLucky" ),
      reliableTalent: (this.system.proficient >= 1) && this.actor.getFlag("mc3e", "reliableTalent"),
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this.actor}),
        "flags.mc3e.roll": {type: "tool", itemId: this.id, itemUuid: this.uuid}
      }
    }, options);
    rollConfig.parts = parts.concat(options.parts ?? []);
     ...
     * @function mc3e.preRollToolCheck
     * @memberof hookEvents
     * @param {Item5e} item                  Item for which the roll is being performed.
     * @param {D20RollConfiguration} config  Configuration data for the pending roll.
     * @returns {boolean}                    Explicitly return false to prevent the roll from being performed.
     */
    if ( Hooks.call("mc3e.preRollToolCheck", this, rollConfig) === false ) return;

    const roll = await d20Roll(rollConfig);

    /**
     * A hook event that fires after a tool check has been rolled for an Item.
     * @function mc3e.rollToolCheck
     * @memberof hookEvents
     * @param {Item5e} item   Item for which the roll was performed.
     * @param {D20Roll} roll  The resulting roll.
     */
    if ( roll ) Hooks.callAll("mc3e.rollToolCheck", this, roll);
  ...
  static async _onChatCardAction(event) {
    ...
    const storedData = message.getFlag("mc3e", "itemData");
    const item = storedData ? new this(storedData, {parent: actor}) : actor.items.get(card.dataset.itemId);
    if ( !item ) {
      const err = game.i18n.format("mc3e.ActionWarningNoItem", {item: card.dataset.itemId, name: actor.name});
    ...
    switch ( action ) {
      ...
      case "placeTemplate":
        try {
          await mc3e.canvas.AbilityTemplate.fromItem(item)?.drawPreview();
        } catch(err) {}
        break;
      case "abilityCheck":
        targets = this._getChatCardTargets(card);
  ...
  static _getChatCardTargets(card) {
    ...
    if ( !targets.length ) ui.notifications.warn(game.i18n.localize("mc3e.ActionWarningNoToken"));
    return targets;
  }
  ...
  createAdvancement(type, data={}, { showConfig=true, source=false }={}) {
    ...
    const Advancement = CONFIG.mc3e.advancementTypes[type];
    if ( !Advancement ) throw new Error(`${type} not found in CONFIG.mc3e.advancementTypes`);
  ...
  async _preUpdate(changed, options, user) {
    ...
    if ( changed.system.levels <= 0 ) {
      ui.notifications.warn(game.i18n.localize("mc3e.MaxClassLevelMinimumWarn"));
      changed.system.levels = 1;
    }

    // Check to make sure the updated class level doesn't exceed level cap
    if ( changed.system.levels > CONFIG.mc3e.maxLevel ) {
      ui.notifications.warn(game.i18n.format("mc3e.MaxClassLevelExceededWarn", {max: CONFIG.mc3e.maxLevel}));
      changed.system.levels = CONFIG.mc3e.maxLevel;
    }
    ...
    if ( newCharacterLevel > CONFIG.mc3e.maxLevel ) {
      ui.notifications.warn(game.i18n.format("mc3e.MaxCharacterLevelExceededWarn", {max: CONFIG.mc3e.maxLevel}));
      changed.system.levels -= newCharacterLevel - CONFIG.mc3e.maxLevel;
    }
  }
  ...
  _onCreateOwnedEquipment(data, isNPC) {
    ...
    if ( foundry.utils.getProperty(data, "system.proficient") === undefined ) {
      if ( isNPC ) {
        ...
        const armorProf = CONFIG.mc3e.armorProficienciesMap[this.system.armor?.type]; // Player characters check proficiency
  ...
  _onCreateOwnedWeapon(data, isNPC) {
    ...
    const weaponProf = CONFIG.mc3e.weaponProficienciesMap[this.system.weaponType];
    const updates = {};
  ...
  static async createScrollFromSpell(spell) {
    ...
    // Get scroll data
    const scrollUuid = `Compendium.${CONFIG.mc3e.sourcePacks.ITEMS}.${CONFIG.mc3e.spellScrollIds[level]}`;
    ...
    const spellScrollData = foundry.utils.mergeObject(scrollData, {
      name: `${game.i18n.localize("mc3e.SpellScroll")}: ${itemData.name}`,
      img: itemData.img,
      system: {
