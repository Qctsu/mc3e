...
export default class Actor5e extends Actor {
   ...
   */
  get isPolymorphed() {
    return this.getFlag("mc3e", "isPolymorphed") || false;
  }
  ...
  _initializeSource(source, options={}) {
    ...
    if ( !source._id || !options.pack || mc3e.moduleArt.suppressArt ) return source;
    const uuid = `Compendium.${options.pack}.${source._id}`;
    const art = game.mc3e.moduleArt.map.get(uuid);
   ...
   */
  getLevelExp(level) {
    const levels = CONFIG.mc3e.CHARACTER_EXP_LEVELS;
  ...
  getCRExp(cr) {
    ...
    return CONFIG.mc3e.CR_EXP_LEVELS[cr];
  }
  ...
  /**
   * Update the actor's abilities list to match the abilities configured in `mc3e.abilities`.
  ...
  _prepareBaseAbilities() {
    ...
    const abilities = {};
    for ( const key of Object.keys(CONFIG.mc3e.abilities) ) {
  ...
  /**
   * Update the actor's skill list to match the skills configured in `mc3e.skills`.
  ...
  _prepareBaseSkills() {
    ...
    const skills = {};
    for ( const [key, skill] of Object.entries(CONFIG.mc3e.skills) ) {
  ...
  _prepareCharacterData() {
    ...
    for ( const item of this.items ) {
      ...
      // Attuned items
      else if ( item.system.attunement === CONFIG.mc3e.attunementTypes.ATTUNED ) {
  ...
  _prepareNPCData() {
    ...
    this.system.attributes.attunement.value = this.items.filter(i => {
      return i.system.attunement === CONFIG.mc3e.attunementTypes.ATTUNED;
    }).length;
  ...
  _prepareSkills(bonusData, globalBonuses, checkBonus, originalSkills) {
    ...
    // Skill modifiers
    const feats = CONFIG.mc3e.characterFlags;
  ...
  _prepareArmorClass() {
    ...
    const armorTypes = new Set(Object.keys(CONFIG.mc3e.armorTypes));
      ...
      default:
        ...
        if ( armors.length ) {
          if ( armors.length > 1 ) this._preparationWarnings.push({
            message: game.i18n.localize("mc3e.WarnMultipleArmor"), type: "warning"
          });
        ...
        try {
          ...
          this._preparationWarnings.push({
            message: game.i18n.localize("mc3e.WarnBadACFormula"), link: "armor", type: "error"
          });
          const replaced = Roll.replaceFormulaData(CONFIG.mc3e.armorClasses.default.formula, rollData);
          ac.base = Roll.safeEval(replaced);
        }
        break;
    }
    ...
    if ( shields.length ) {
      if ( shields.length > 1 ) this._preparationWarnings.push({
        message: game.i18n.localize("mc3e.WarnMultipleShields"), type: "warning"
      });
  ...
  _prepareEncumbrance() {
    ...
    if ( game.settings.get("mc3e", "currencyWeight") && currency ) {
      const numCoins = Object.values(currency).reduce((val, denom) => val + Math.max(denom, 0), 0);
      const currencyPerWeight = game.settings.get("mc3e", "metricWeightUnits")
        ? CONFIG.mc3e.encumbrance.currencyPerWeight.metric
        : CONFIG.mc3e.encumbrance.currencyPerWeight.imperial;
      weight += numCoins / currencyPerWeight;
    }

    // Determine the Encumbrance size class
    let mod = {tiny: 0.5, sm: 1, med: 1, lg: 2, huge: 4, grg: 8}[this.system.traits.size] || 1;
    if ( this.flags.mc3e?.powerfulBuild ) mod = Math.min(mod * 2, 8);

    const strengthMultiplier = game.settings.get("mc3e", "metricWeightUnits")
      ? CONFIG.mc3e.encumbrance.strMultiplier.metric
      : CONFIG.mc3e.encumbrance.strMultiplier.imperial;
  ...
  _prepareHitPoints(rollData) {
    ...
    const abilityId = CONFIG.mc3e.hitPointsAbility || "con";
  ...
  _prepareInitiative(bonusData, globalCheckBonus=0) {
    ...
    const abilityId = init.ability || CONFIG.mc3e.initiativeAbility;
  ...
  _prepareSpellcasting() {
    ...
    for ( const type of Object.keys(CONFIG.mc3e.spellcastingTypes) ) {
  ...
  static computeClassProgression(progression, cls, {actor, spellcasting, count=1}={}) {
     ...
     * The actual hook names include the spellcasting type (e.g. `mc3e.computeLeveledProgression`).
     * @param {object} progression                    Spellcasting progression data. *Will be mutated.*
     ...
     * @function mc3e.computeSpellcastingProgression
     * @memberof hookEvents
     */
    const allowed = Hooks.call(
      `mc3e.compute${type.capitalize()}Progression`, progression, actor, cls, spellcasting, count
    );
  ...
  static computeLeveledProgression(progression, actor, cls, spellcasting, count) {
    const prog = CONFIG.mc3e.spellcastingTypes.leveled.progression[spellcasting.progression];
    if ( !prog ) return;
  ...
  static prepareSpellcastingSlots(spells, type, progression, {actor}={}) {
     ...
     * The actual hook names include the spellcasting type (e.g. `mc3e.prepareLeveledSlots`).
     * @param {object} spells        The `data.spells` object within actor's data. *Will be mutated.*
     * @param {Actor5e} actor        Actor for whom the data is being prepared.
     * @param {object} progression   Spellcasting progression data.
     * @returns {boolean}            Explicitly return false to prevent default preparation from being performed.
     * @function mc3e.prepareSpellcastingSlots
     * @memberof hookEvents
     */
    const allowed = Hooks.call(`mc3e.prepare${type.capitalize()}Slots`, spells, actor, progression);
  ...
  static prepareLeveledSlots(spells, actor, progression) {
    const levels = Math.clamped(progression.slot, 0, CONFIG.mc3e.maxLevel);
    const slots = CONFIG.mc3e.SPELL_SLOT_TABLE[Math.min(levels, CONFIG.mc3e.SPELL_SLOT_TABLE.length) - 1] ?? [];
  ...
  static preparePactSlots(spells, actor, progression) {
    ...
    let pactLevel = Math.clamped(progression.pact, 0, CONFIG.mc3e.maxLevel);
    spells.pact ??= {};
  ...
  async _preCreate(data, options, user) {
    ...
    if ( "size" in (this.system.traits || {}) ) {
      const s = CONFIG.mc3e.tokenSizes[this.system.traits.size || "med"];
  ...
  async _preUpdate(changed, options, user) {
    ...
    if ( "size" in (this.system.traits || {}) ) {
      ...
      if ( newSize && (newSize !== this.system.traits?.size) ) {
        let size = CONFIG.mc3e.tokenSizes[newSize];
  ...
  _isRemarkableAthlete(ability) {
    return this.getFlag("mc3e", "remarkableAthlete")
      && CONFIG.mc3e.characterFlags.remarkableAthlete.abilities.includes(ability);
  }
  ...
  async rollSkill(skillId, options={}) {
    ...
    const reliableTalent = (skl.value >= 1 && this.getFlag("mc3e", "reliableTalent"));

    // Roll and return
    const flavor = game.i18n.format("mc3e.SkillPromptTitle", {skill: CONFIG.mc3e.skills[skillId]?.label ?? ""});
    const rollData = foundry.utils.mergeObject({
      data: data,
      title: `${flavor}: ${this.name}`,
      flavor,
      chooseModifier: true,
      halflingLucky: this.getFlag("mc3e", "halflingLucky"),
      reliableTalent,
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.mc3e.roll": {type: "skill", skillId }
      }
    }, options);
     ...
     * @function mc3e.preRollSkill
     * @memberof hookEvents
     * @param {Actor5e} actor                Actor for which the skill check is being rolled.
     * @param {D20RollConfiguration} config  Configuration data for the pending roll.
     * @param {string} skillId               ID of the skill being rolled as defined in `mc3e.skills`.
     * @returns {boolean}                    Explicitly return `false` to prevent skill check from being rolled.
     */
    if ( Hooks.call("mc3e.preRollSkill", this, rollData, skillId) === false ) return;

    const roll = await d20Roll(rollData);

    /**
     * A hook event that fires after a skill check has been rolled for an Actor.
     * @function mc3e.rollSkill
     * @memberof hookEvents
     * @param {Actor5e} actor   Actor for which the skill check has been rolled.
     * @param {D20Roll} roll    The resulting roll.
     * @param {string} skillId  ID of the skill that was rolled as defined in `mc3e.skills`.
     */
    if ( roll ) Hooks.callAll("mc3e.rollSkill", this, roll, skillId);
  ...
  rollAbility(abilityId, options={}) {
    const label = CONFIG.mc3e.abilities[abilityId] ?? "";
    new Dialog({
      title: `${game.i18n.format("mc3e.AbilityPromptTitle", {ability: label})}: ${this.name}`,
      content: `<p>${game.i18n.format("mc3e.AbilityPromptText", {ability: label})}</p>`,
      buttons: {
        test: {
          label: game.i18n.localize("mc3e.ActionAbil"),
          callback: () => this.rollAbilityTest(abilityId, options)
        },
        save: {
          label: game.i18n.localize("mc3e.ActionSave"),
  ...
  async rollAbilityTest(abilityId, options={}) {
    ...
    // Roll and return
    const flavor = game.i18n.format("mc3e.AbilityPromptTitle", {ability: label});
    const rollData = foundry.utils.mergeObject({
      data,
      title: `${flavor}: ${this.name}`,
      flavor,
      ...
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.mc3e.roll": {type: "ability", abilityId }
      }
    }, options);
    rollData.parts = parts.concat(options.parts ?? []);
     ...
     * @function mc3e.preRollAbilityTest
     * @memberof hookEvents
     * @param {Actor5e} actor                Actor for which the ability test is being rolled.
     * @param {D20RollConfiguration} config  Configuration data for the pending roll.
     * @param {string} abilityId             ID of the ability being rolled as defined in `mc3e.abilities`.
     * @returns {boolean}                    Explicitly return `false` to prevent ability test from being rolled.
     */
    if ( Hooks.call("mc3e.preRollAbilityTest", this, rollData, abilityId) === false ) return;

    const roll = await d20Roll(rollData);

    /**
     * A hook event that fires after an ability test has been rolled for an Actor.
     * @function mc3e.rollAbilityTest
     * @memberof hookEvents
     * @param {Actor5e} actor     Actor for which the ability test has been rolled.
     * @param {D20Roll} roll      The resulting roll.
     * @param {string} abilityId  ID of the ability that was rolled as defined in `mc3e.abilities`.
     */
    if ( roll ) Hooks.callAll("mc3e.rollAbilityTest", this, roll, abilityId);
  ...
  async rollAbilitySave(abilityId, options={}) {
    ...
    // Roll and return
    const flavor = game.i18n.format("mc3e.SavePromptTitle", {ability: label});
    const rollData = foundry.utils.mergeObject({
      data,
      title: `${flavor}: ${this.name}`,
      flavor,
      ...
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.mc3e.roll": {type: "save", abilityId }
      }
    }, options);
    rollData.parts = parts.concat(options.parts ?? []);
     ...
     * @function mc3e.preRollAbilitySave
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("mc3e.preRollAbilitySave", this, rollData, abilityId) === false ) return;

    const roll = await d20Roll(rollData);

    /**
     * A hook event that fires after an ability save has been rolled for an Actor.
     * @function mc3e.rollAbilitySave
     * @memberof hookEvents
     * @param {Actor5e} actor     Actor for which the ability save has been rolled.
     * @param {D20Roll} roll      The resulting roll.
     * @param {string} abilityId  ID of the ability that was rolled as defined in `mc3e.abilities`.
     */
    if ( roll ) Hooks.callAll("mc3e.rollAbilitySave", this, roll, abilityId);
  ...
  async rollDeathSave(options={}) {
    ...
    if ( (this.system.attributes.hp.value > 0) || (death.failure >= 3) || (death.success >= 3) ) {
      ui.notifications.warn(game.i18n.localize("mc3e.DeathSaveUnnecessary"));
      return null;
    }
    ...
    if ( this.getFlag("mc3e", "diamondSoul") ) {
      parts.push("@prof");
    ...
    // Evaluate the roll
    const flavor = game.i18n.localize("mc3e.DeathSavingThrow");
    const rollData = foundry.utils.mergeObject({
      data,
      title: `${flavor}: ${this.name}`,
      flavor,
     ...
     * @function mc3e.preRollDeathSave
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("mc3e.preRollDeathSave", this, rollData) === false ) return;
    ...
    if ( roll.total >= (roll.options.targetValue ?? 10) ) {
      ...
      if ( roll.isCritical ) {
        ...
        details.chatString = "mc3e.DeathSaveCriticalSuccess";
      }
      ...
      // 3 Successes = survive and reset checks
        ...
        details.chatString = "mc3e.DeathSaveSuccess";
      }
    ...
    // Save failure
      ...
      if ( failures >= 3 ) {  // 3 Failures = death
        details.chatString = "mc3e.DeathSaveFailure";
      }
    }
     ...
     */
    if ( Hooks.call("mc3e.rollDeathSave", this, roll, details) === false ) return roll;
  ...
  getInitiativeRoll(options={}) {
    ...
    const data = this.getRollData();
    const flags = this.flags.mc3e || {};
    if ( flags.initiativeAdv ) options.advantageMode ??= mc3e.dice.D20Roll.ADV_MODE.ADVANTAGE;
    ...
    const tiebreaker = game.settings.get("mc3e", "initiativeDexTiebreaker");
    ...
    options = foundry.utils.mergeObject({
      flavor: options.flavor ?? game.i18n.localize("mc3e.Initiative"),
  ...
  async rollInitiativeDialog(rollOptions={}) {
    ...
    const choice = await roll.configureDialog({
      ...
      title: `${game.i18n.localize("mc3e.InitiativeRoll")}: ${this.name}`,
      chooseModifier: false,
      defaultAction: rollOptions.advantageMode ?? mc3e.dice.D20Roll.ADV_MODE.NORMAL
    });
  ...
  async rollInitiative(options={}) {
     ...
     * @function mc3e.preRollInitiative
     * @memberof hookEvents
     * @param {Actor5e} actor  The Actor that is rolling initiative.
     * @param {D20Roll} roll   The initiative roll.
     */
    if ( Hooks.call("mc3e.preRollInitiative", this, this._cachedInitiativeRoll) === false ) return;
  ...
  async rollHitDie(denomination, options={}) {
    ...
    if ( !cls ) {
      ui.notifications.error(game.i18n.format("mc3e.HitDiceWarn", {name: this.name, formula: denomination}));
      return null;
    }

    // Prepare roll data
    const flavor = game.i18n.localize("mc3e.HitDiceRoll");
     ...
     */
    if ( Hooks.call("mc3e.preRollHitDie", this, rollConfig, denomination) === false ) return;
     ...
     * @returns {boolean}             Explicitly return `false` to prevent updates from being performed.
     */
    if ( Hooks.call("mc3e.rollHitDie", this, roll, updates) === false ) return roll;
   ...
   * @see {@link mc3e.preRollClassHitPoints}
   */
  async rollClassHitPoints(item, { chatMessage=true }={}) {
    ...
    const flavor = game.i18n.format("mc3e.AdvancementHitPointsRollMessage", { class: item.name });
    const messageData = {
      title: `${flavor}: ${this.name}`,
      flavor,
      speaker: ChatMessage.getSpeaker({ actor: this }),
      "flags.mc3e.roll": { type: "hitPoints" }
    };

    /**
     * A hook event that fires before hit points are rolled for a character's class.
     * @function mc3e.preRollClassHitPoints
     * @memberof hookEvents
     ...
     */
    Hooks.callAll("mc3e.preRollClassHitPoints", this, item, rollData, messageData);
     ...
     * @function mc3e.rollClassHitPoints
     * @memberof hookEvents
     * @param {Actor5e} actor  Actor for which the hit points have been rolled.
     * @param {Roll} roll      The resulting roll.
     */
    Hooks.callAll("mc3e.rollClassHitPoints", this, roll);
   ...
   * @param {boolean} [options.chatMessage=true]  Display the chat message for this roll.
   * @returns {Promise<Roll>}                     The completed roll.
   * @see {@link mc3e.preRollNPCHitPoints}
   */
  async rollNPCHitPoints({ chatMessage=true }={}) {
    ...
    const flavor = game.i18n.format("mc3e.HPFormulaRollMessage");
    const messageData = {
      title: `${flavor}: ${this.name}`,
      flavor,
      speaker: ChatMessage.getSpeaker({ actor: this }),
     ...
     * @function mc3e.preRollNPCHitPoints
     * @memberof hookEvents
     * @param {Actor5e} actor            Actor for which the hit points are being rolled.
     * @param {object} rollData
     * @param {string} rollData.formula  The string formula to parse.
     ...
     * @param {object} messageData       The data object to use when creating the message.
     */
    Hooks.callAll("mc3e.preRollNPCHitPoints", this, rollData, messageData);
     ...
     * @function mc3e.rollNPCHitPoints
     * @memberof hookEvents
     * @param {Actor5e} actor  Actor for which the hit points have been rolled.
     * @param {Roll} roll      The resulting roll.
     */
    Hooks.callAll("mc3e.rollNPCHitPoints", this, roll);
  ...
  async shortRest(config={}) {
     ...
     * @function mc3e.preShortRest
     * @memberof hookEvents
     * @param {Actor5e} actor             The actor that is being rested.
     * @param {RestConfiguration} config  Configuration options for the rest.
     * @returns {boolean}                 Explicitly return `false` to prevent the rest from being started.
     */
    if ( Hooks.call("mc3e.preShortRest", this, config) === false ) return;
  ...
  async longRest(config={}) {
     ...
     * @function mc3e.preLongRest
     * @memberof hookEvents
     * @param {Actor5e} actor             The actor that is being rested.
     * @param {RestConfiguration} config  Configuration options for the rest.
     * @returns {boolean}                 Explicitly return `false` to prevent the rest from being started.
     */
    if ( Hooks.call("mc3e.preLongRest", this, config) === false ) return;
  ...
  async _rest(chat, newDay, longRest, dhd=0, dhp=0) {
     ...
     * @function mc3e.preRestCompleted
     * @memberof hookEvents
     * @param {Actor5e} actor      The actor that is being rested.
     * @param {RestResult} result  Details on the rest to be completed.
     * @returns {boolean}          Explicitly return `false` to prevent the rest updates from being performed.
     */
    if ( Hooks.call("mc3e.preRestCompleted", this, result) === false ) return result;
     ...
     * @function mc3e.restCompleted
     * @memberof hookEvents
     * @param {Actor5e} actor      The actor that just completed resting.
     * @param {RestResult} result  Details on the rest completed.
     */
    Hooks.callAll("mc3e.restCompleted", this, result);
  ...
  async _displayRestResultMessage(result, longRest=false) {
    ...
    let restFlavor;
    switch (game.settings.get("mc3e", "restVariant")) {
      case "normal":
        restFlavor = (longRest && newDay) ? "mc3e.LongRestOvernight" : `mc3e.${length}RestNormal`;
        break;
      case "gritty":
        restFlavor = (!longRest && newDay) ? "mc3e.ShortRestOvernight" : `mc3e.${length}RestGritty`;
        break;
      case "epic":
        restFlavor = `mc3e.${length}RestEpic`;
        break;
    }

    // Determine the chat message to display
    let message;
    if ( diceRestored && healthRestored ) message = `mc3e.${length}RestResult`;
    else if ( longRest && !diceRestored && healthRestored ) message = "mc3e.LongRestResultHitPoints";
    else if ( longRest && diceRestored && !healthRestored ) message = "mc3e.LongRestResultHitDice";
    else message = `mc3e.${length}RestResultShort`;
  ...
  async _getRestItemUsesRecovery({recoverShortRestUses=true, recoverLongRestUses=true,
    ...
    for ( let item of this.items ) {
      ...
      if ( recoverDailyUses && uses?.recovery && (uses?.per === "charges") ) {
        ...
        if ( recoverLongRestUses && (game.settings.get("mc3e", "restVariant") === "gritty") ) {
        ...
        try {
          ...
          ui.notifications.warn(game.i18n.format("mc3e.ItemRecoveryFormulaWarning", {
            name: item.name,
        ...
        if ( newValue !== uses.value ) {
          ...
          const locKey = `mc3e.Item${diff < 0 ? "Loss" : "Recovery"}Roll${isMax ? "Max" : ""}`;
   ...
   * See CONFIG.mc3e.currencies for configuration.
   * @returns {Promise<Actor5e>}
   */
  convertCurrency() {
    const currency = foundry.utils.deepClone(this.system.currency);
    const currencies = Object.entries(CONFIG.mc3e.currencies);
  ...
  async transformInto(target, { keepPhysical=false, keepMental=false, keepSaves=false, keepSkills=false,
    ...
    const allowed = game.settings.get("mc3e", "allowPolymorphing");
    if ( !allowed && !game.user.isGM ) {
      return ui.notifications.warn(game.i18n.localize("mc3e.PolymorphWarn"));
    }

    // Get the original Actor data and the new source data
    const o = this.toObject();
    o.flags.mc3e = o.flags.mc3e || {};
    o.flags.mc3e.transformOptions = {mergeSkills, mergeSaves};
    const source = target.toObject();

    if ( keepSelf ) {
      o.img = source.img;
      o.name = `${o.name} (${game.i18n.localize("mc3e.PolymorphSelf")})`;
    }
    ...
    if ( !keepSelf ) {
      ...
      if ( !keepClass && d.system.details.cr ) {
        const cls = new mc3e.dataModels.item.ClassData({levels: d.system.details.cr});
        d.items.push({
          type: "class",
          name: game.i18n.localize("mc3e.PolymorphTmpClass"),
          system: cls.toObject()
        });
      }
    ...
    // Set new data flags
    if ( !this.isPolymorphed || !d.flags.mc3e.originalActor ) d.flags.mc3e.originalActor = this.id;
    d.flags.mc3e.isPolymorphed = true;

    // Gather previous actor data
    const previousActorIds = this.getFlag("mc3e", "previousActorIds") || [];
    previousActorIds.push(this._id);
    foundry.utils.setProperty(d.flags, "mc3e.previousActorIds", previousActorIds);
    ...
    if ( this.isToken ) {
      ...
      setProperty(tokenData, "flags.mc3e.previousActorData", this.token.toObject().actorData);
     ...
     * @param {object} [options]
     */
    Hooks.callAll("mc3e.transformActor", this, target, d, {
    ...
    const updates = tokens.map(t => {
      ...
      const dOriginalActor = foundry.utils.getProperty(d, "flags.mc3e.originalActor");
      foundry.utils.setProperty(newTokenData, "flags.mc3e.originalActor", dOriginalActor);
      foundry.utils.setProperty(newTokenData, "flags.mc3e.isPolymorphed", true);
      return newTokenData;
    });
  ...
  async revertOriginalForm({renderSheet=true}={}) {
    ...
    if ( !this.isOwner ) return ui.notifications.warn(game.i18n.localize("mc3e.PolymorphRevertWarn"));

    /**
     * A hook event that fires just before the actor is reverted to original form.
     * @function mc3e.revertOriginalForm
     * @memberof hookEvents
     * @param {Actor} this                 The original actor before transformation.
     * @param {object} [options]
     */
    Hooks.callAll("mc3e.revertOriginalForm", this, {renderSheet});
    const previousActorIds = this.getFlag("mc3e", "previousActorIds") ?? [];
    const isOriginalActor = !previousActorIds.length;
    const isRendered = this.sheet.rendered;

    // Obtain a reference to the original actor
    const original = game.actors.get(this.getFlag("mc3e", "originalActor"));
    ...
    if ( this.isToken ) {
      ...
      if ( !baseActor ) {
        ui.notifications.warn(game.i18n.format("mc3e.PolymorphRevertNoOriginalActorWarn", {
          reference: this.getFlag("mc3e", "originalActor")
        }));
        return;
      }
      const prototypeTokenData = await baseActor.getTokenDocument();
      const actorData = this.token.getFlag("mc3e", "previousActorData");
      ...
      if ( isOriginalActor ) {
        await this.unsetFlag("mc3e", "isPolymorphed");
        await this.unsetFlag("mc3e", "previousActorIds");
        await this.token.unsetFlag("mc3e", "previousActorData");
      }
  ...
  static addDirectoryContextOptions(html, entryOptions) {
    entryOptions.push({
      name: "mc3e.PolymorphRestoreTransformation",
  ...
  static formatCreatureType(typeData) {
    ...
    if ( typeData.value === "custom" ) {
      ...
      let code = CONFIG.mc3e.creatureTypes[typeData.value];
      localizedType = game.i18n.localize(typeData.swarm ? `${code}Pl` : code);
    }
    let type = localizedType;
    if ( typeData.swarm ) {
      type = game.i18n.format("mc3e.CreatureSwarmPhrase", {
        size: game.i18n.localize(CONFIG.mc3e.actorSizes[typeData.swarm]),
        type: localizedType
      });
    }
  ...
  _displayScrollingDamage(dhp) {
    ...
    for ( const t of tokens ) {
      ...
      canvas.interface.createScrollingText(t.center, dhp.signedString(), {
        ...
        fill: CONFIG.mc3e.tokenHPColors[dhp < 0 ? "damage" : "healing"],
        stroke: 0x000000,
        strokeThickness: 4,
        jitter: 0.25
      });
   ...
   * @returns {object}
   * @private
   * @deprecated since mc3e 2.0, targeted for removal in 2.2
   */
  _computeArmorClass() {
  ...
  _simplifyBonus(bonus, data) {
    foundry.utils.logCompatibilityWarning(
      "Actor#_simplifyBonus has been made a utility function and can be accessed at mc3e.utils.simplifyBonus.",
      { since: "mc3e 2.0", until: "mc3e 2.2" }
    );
