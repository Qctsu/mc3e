 ...
 * @param {string} trait  Trait as defined in `CONFIG.mc3e.traits`.
 * @returns {object}      Trait categories defined within `CONFIG.mc3e`.
 */
export function categories(trait) {
  const traitConfig = CONFIG.mc3e.traits[trait];
  return CONFIG.mc3e[traitConfig.configKey ?? trait];
}
...
export async function choices(trait, chosen=new Set()) {
  ...
  if ( traitConfig.children ) {
    for ( const [categoryKey, childrenKey] of Object.entries(traitConfig.children) ) {
      const children = CONFIG.mc3e[childrenKey];
  ...
  if ( traitConfig.subtypes ) {
    ...
    const map = CONFIG.mc3e[`${trait}ProficienciesMap`];

    // Merge all IDs lists together
    const ids = traitConfig.subtypes.ids.reduce((obj, key) => {
      if ( CONFIG.mc3e[key] ) Object.assign(obj, CONFIG.mc3e[key]);
      return obj;
    }, {});
  ...
  // Sort Categories
  if ( traitConfig.sortCategories ) data = mc3e.utils.sortObjectEntries(data, "label");

  // Sort Children
  for ( const category of Object.values(data) ) {
    if ( !category.children ) continue;
    category.children = mc3e.utils.sortObjectEntries(category.children, "label");
  }
...
export function getBaseItem(identifier, { indexOnly=false, fullItem=false }={}) {
  let pack = CONFIG.mc3e.sourcePacks.ITEMS;
...
export function traitIndexFields() {
  const fields = [];
  for ( const traitConfig of Object.values(CONFIG.mc3e.traits) ) {
...
export function traitLabel(trait, count) {
  ...
  return game.i18n.localize(`mc3e.Trait${typeCap}Plural.${pluralRule}`);
}
...
export function keyLabel(trait, key) {
  ...
  for ( const childrenKey of Object.values(traitConfig.children ?? {}) ) {
    if ( CONFIG.mc3e[childrenKey]?.[key] ) return CONFIG.mc3e[childrenKey]?.[key];
  }

  for ( const idsKey of traitConfig.subtypes?.ids ?? [] ) {
    if ( !CONFIG.mc3e[idsKey]?.[key] ) continue;
    const index = getBaseItem(CONFIG.mc3e[idsKey][key], { indexOnly: true });
...
export function choiceLabel(trait, choice) {
  ...
  if ( !choice.pool ) {
    return game.i18n.format("mc3e.TraitConfigChooseAny", {
      count: choice.count,
  ...
  return game.i18n.format("mc3e.TraitConfigChooseList", {
    count: choice.count,
