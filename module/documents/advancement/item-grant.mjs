...
export default class ItemGrantAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      order: 40,
      icon: "systems/mc3e/icons/svg/item-grant.svg",
      title: game.i18n.localize("mc3e.AdvancementItemGrantTitle"),
      hint: game.i18n.localize("mc3e.AdvancementItemGrantHint"),
      apps: {
  ...
  summaryForLevel(level, { configMode=false }={}) {
    ...
    if ( !this.value.added || configMode ) {
      return this.configuration.items.reduce((html, uuid) => html + mc3e.utils.linkForUuid(uuid), "");
    }
    ...
    for ( const [uuid, selected] of Object.entries(data) ) {
      ...
      if ( !itemData ) {
        ...
        itemData = source.clone({
          ...
          "flags.mc3e.sourceId": uuid,
          "flags.mc3e.advancementOrigin": `${this.item.id}.${this.id}`
  ...
  restore(level, data) {
    ...
    for ( const item of data.items ) {
      ...
      updates[item._id] = item.flags.mc3e.sourceId;
    }
    this.updateSource({"value.added": updates});
  }
