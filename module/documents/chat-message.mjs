...
export function highlightCriticalSuccessFailure(message, html, data) {
  ...
  if ( !d20Roll ) return;
  d20Roll = mc3e.dice.D20Roll.fromRoll(d20Roll);
...
export function displayChatActionButtons(message, html, data) {
  const chatCard = html.find(".mc3e.chat-card");
...
export function addChatMessageContextOptions(html, options) {
  ...
  options.push(
    {
      name: game.i18n.localize("mc3e.ChatContextDamage"),
      icon: '<i class="fas fa-user-minus"></i>',
      condition: canApply,
      callback: li => applyChatCardDamage(li, 1)
    },
    {
      name: game.i18n.localize("mc3e.ChatContextHealing"),
      icon: '<i class="fas fa-user-plus"></i>',
      condition: canApply,
      callback: li => applyChatCardDamage(li, -1)
    },
    {
      name: game.i18n.localize("mc3e.ChatContextTempHP"),
      icon: '<i class="fas fa-user-clock"></i>',
      condition: canApply,
      callback: li => applyChatCardTemp(li)
    },
    {
      name: game.i18n.localize("mc3e.ChatContextDoubleDamage"),
      icon: '<i class="fas fa-user-injured"></i>',
      condition: canApply,
      callback: li => applyChatCardDamage(li, 2)
    },
    {
      name: game.i18n.localize("mc3e.ChatContextHalfDamage"),
...
export function onRenderChatMessage(app, html, data) {
  ...
  if (game.settings.get("mc3e", "autoCollapseItemCards")) html.find(".card-content").hide();
}
