 ...
 * @property {string} activation.type       Activation type as defined in `mc3e.abilityActivationTypes`.
 ...
 * @property {string} duration.units        Time duration period as defined in `mc3e.timePeriods`.
 ...
 * @property {string} target.units          Units used for value and width as defined in `mc3e.distanceUnits`.
 * @property {string} target.type           Targeting mode as defined in `mc3e.targetTypes`.
 ...
 * @property {string} range.units           Units used for value and long as defined in `mc3e.distanceUnits`.
 ...
 * @property {string} uses.per              Recharge time for limited uses as defined in `mc3e.limitedUsePeriods`.
 * @property {object} consume               Effect's resource consumption.
 * @property {string} consume.type          Type of resource to consume as defined in `mc3e.abilityConsumptionTypes`.
  ...
  static defineSchema() {
    return {
      activation: new foundry.data.fields.SchemaField({
        type: new foundry.data.fields.StringField({required: true, blank: true, label: "mc3e.ItemActivationType"}),
        cost: new foundry.data.fields.NumberField({required: true, label: "mc3e.ItemActivationCost"}),
        condition: new foundry.data.fields.StringField({required: true, label: "mc3e.ItemActivationCondition"})
      }, {label: "mc3e.ItemActivation"}),
      duration: new foundry.data.fields.SchemaField({
        value: new FormulaField({required: true, deterministic: true, label: "mc3e.Duration"}),
        units: new foundry.data.fields.StringField({required: true, blank: true, label: "mc3e.DurationType"})
      }, {label: "mc3e.Duration"}),
      target: new foundry.data.fields.SchemaField({
        value: new foundry.data.fields.NumberField({required: true, min: 0, label: "mc3e.TargetValue"}),
        width: new foundry.data.fields.NumberField({required: true, min: 0, label: "mc3e.TargetWidth"}),
        units: new foundry.data.fields.StringField({required: true, blank: true, label: "mc3e.TargetUnits"}),
        type: new foundry.data.fields.StringField({required: true, blank: true, label: "mc3e.TargetType"})
      }, {label: "mc3e.Target"}),
      range: new foundry.data.fields.SchemaField({
        value: new foundry.data.fields.NumberField({required: true, min: 0, label: "mc3e.RangeNormal"}),
        long: new foundry.data.fields.NumberField({required: true, min: 0, label: "mc3e.RangeLong"}),
        units: new foundry.data.fields.StringField({required: true, blank: true, label: "mc3e.RangeUnits"})
      }, {label: "mc3e.Range"}),
      uses: new this.ItemUsesField({}, {label: "mc3e.LimitedUses"}),
      consume: new foundry.data.fields.SchemaField({
        type: new foundry.data.fields.StringField({required: true, blank: true, label: "mc3e.ConsumeType"}),
        target: new foundry.data.fields.StringField({
          required: true, nullable: true, initial: null, label: "mc3e.ConsumeTarget"
        }),
        amount: new foundry.data.fields.NumberField({required: true, integer: true, label: "mc3e.ConsumeAmount"})
      }, {label: "mc3e.ConsumeTitle"})
    };
  }
    ...
    constructor(extraSchema, options) {
      super(SystemDataModel.mergeSchema({
        value: new foundry.data.fields.NumberField({
          required: true, min: 0, integer: true, label: "mc3e.LimitedUsesAvailable"
        }),
        max: new FormulaField({required: true, deterministic: true, label: "mc3e.LimitedUsesMax"}),
        per: new foundry.data.fields.StringField({
          required: true, nullable: true, initial: null, label: "mc3e.LimitedUsesPer"
        }),
        recovery: new FormulaField({required: true, label: "mc3e.RecoveryFormula"})
      }, extraSchema), options);
    }
  };
