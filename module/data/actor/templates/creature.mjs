...
export default class CreatureTemplate extends CommonTemplate {
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      bonuses: new foundry.data.fields.SchemaField({
        mwak: makeAttackBonuses({label: "mc3e.BonusMWAttack"}),
        rwak: makeAttackBonuses({label: "mc3e.BonusRWAttack"}),
        msak: makeAttackBonuses({label: "mc3e.BonusMSAttack"}),
        rsak: makeAttackBonuses({label: "mc3e.BonusRSAttack"}),
        abilities: new foundry.data.fields.SchemaField({
          check: new FormulaField({required: true, label: "mc3e.BonusAbilityCheck"}),
          save: new FormulaField({required: true, label: "mc3e.BonusAbilitySave"}),
          skill: new FormulaField({required: true, label: "mc3e.BonusAbilitySkill"})
        }, {label: "mc3e.BonusAbility"}),
        spell: new foundry.data.fields.SchemaField({
          dc: new FormulaField({required: true, deterministic: true, label: "mc3e.BonusSpellDC"})
        }, {label: "mc3e.BonusSpell"})
      }, {label: "mc3e.Bonuses"}),
      skills: new MappingField(new foundry.data.fields.SchemaField({
        value: new foundry.data.fields.NumberField({required: true, initial: 0, label: "mc3e.ProficiencyLevel"}),
        ability: new foundry.data.fields.StringField({required: true, initial: "dex", label: "mc3e.Ability"}),
        bonuses: new foundry.data.fields.SchemaField({
          check: new FormulaField({required: true, label: "mc3e.SkillBonusCheck"}),
          passive: new FormulaField({required: true, label: "mc3e.SkillBonusPassive"})
        }, {label: "mc3e.SkillBonuses"})
      }), {initialKeys: CONFIG.mc3e.skills, initialValue: this._initialSkillValue}),
      spells: new MappingField(new foundry.data.fields.SchemaField({
        value: new foundry.data.fields.NumberField({
          nullable: false, integer: true, min: 0, initial: 0, label: "mc3e.SpellProfAvailable"
        }),
        override: new foundry.data.fields.NumberField({
          integer: true, min: 0, label: "mc3e.SpellProgOverride"
        })
      }), {initialKeys: this._spellLevels, label: "mc3e.SpellLevels"})
    });
  }
  ...
  static _initialSkillValue(key, initial) {
    if ( CONFIG.mc3e.skills[key]?.ability ) initial.ability = CONFIG.mc3e.skills[key].ability;
    return initial;
  }
  ...
  static get _spellLevels() {
    const levels = Object.keys(CONFIG.mc3e.spellLevels).filter(a => a !== "0").map(l => `spell${l}`);
    return [...levels, "pact"];
  }
...
function makeAttackBonuses(schemaOptions={}) {
  return new foundry.data.fields.SchemaField({
    attack: new FormulaField({required: true, label: "mc3e.BonusAttack"}),
    damage: new FormulaField({required: true, label: "mc3e.BonusDamage"})
  }, schemaOptions);
}
